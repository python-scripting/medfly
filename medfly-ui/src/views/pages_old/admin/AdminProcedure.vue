      ...
      <div class="flex">
        ...
        <form>
          ...
          <div class="flex mb-4">
            ...
            <div class="w-3/4 h-12"><vs-input v-model="pname" class="w-full mb-3"/></div>
...
export default {
  ...
  created() {
    ...
    this.get_allprocedures()
  },
   methods:{
     get_allprocedures(){
        axios.get('http://127.0.0.1:8081/procedures/').then(response => { 
                //alert(JSON.stringify(response))  
                for (var y in response.data){
                  //alert(response.data[y])
                     this.procedures.push(response.data[y])
                }
        })
    ...
    get_alldepartments(){
      ...
      axios.get(`http://127.0.0.1:8081/department/`).then(response => {   
        for (var y in response.data) {

        //this.procedures.push(response.data[y].title)
        //alert(response.data[y].Department_Name)
          this.departmentoptions.push(response.data[y].Department_Name)
        }

      })
    },
    saveloader(){
      axios.post('http://127.0.0.1:8081/procedures/', {
        headers: {
