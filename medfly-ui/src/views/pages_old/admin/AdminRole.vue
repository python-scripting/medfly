      ...
      <vs-tr :data="tr" :key="indextr" v-for="(tr, indextr) in data">
        ...
        <vs-td>
          <vs-row>
            ...
            <vs-button  type="filled" color="rgb(255,20,99)" @click="deleteRole(data[indextr].id)" size="small" icon-pack="feather" icon="icon-x"></vs-button>
          </vs-row>
        </vs-td>
...
export default {
  ...
  methods:{
    ...
    loadRoles(){
      this.roles =[]
       axios.get(`http://127.0.0.1:8081/roles/`).then(response => { 
                     for (var y in response.data) {
        //this.procedures.push(response.data[y].title)
                        this.roles.push(response.data[y])
                     }
       })
        //this.roles.push()
    },
    ...
    saveloader(){
       ...
       axios.post(`http://127.0.0.1:8081/roles/`,{
        headers: {
   ...
   deleteRole(rid){
      ...
      axios.delete(`http://127.0.0.1:8081/roles/`,{
    params: {
      roleid: rid
    }
    }).then(response => {
