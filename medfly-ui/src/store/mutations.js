...
const mutations = {
  ...
  auth_success(state, loginRes){

     // User login seuccess Data (loginRes), Return  From actions.userLogin() function! 
    //state.AppActiveUser['displayName'] = loginRes.user
    //state.AppActiveUser['uid'] = loginRes.id
    localStorage.setItem("user",  JSON.stringify(loginRes))
    state.status = 'success'
    state.token = loginRes.access //
    state.user = loginRes

     
    state.AppActiveUser= {
  uid         : 0,          // From Auth
  displayName : "Raemsh", // From Auth
